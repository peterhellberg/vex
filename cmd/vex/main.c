...
typedef struct {
  ...
  uint16_t lfsr;
  float noise_raw; // last raw LFSR output (+1/-1)
  float noise_lp;  // one-pole lowpassed noise value
...
// inside this band.
#define TONE_NOISE_CLK_MIN 8000.0f
#define TONE_NOISE_CLK_MAX 48000.0f
...
static void mix_callback(void *buffer, unsigned int frames) {
  ...
  for (unsigned int pos = 0; pos < frames; pos++) {
    ...
    for (int ch = 0; ch < VEX_TONE_CHANNELS; ch++) {
      ...
      case 1: { // noise: 15-bit LFSR, clock clamped to the crisp band
        ...
        while (v->nph >= 1.0f) {
          ...
          v->noise_raw = (v->lfsr & 1) ? 1.0f : -1.0f;
        }
        ...
        v->noise_lp += 0.18f * (v->noise_raw - v->noise_lp);
        s = v->noise_lp * 1.4f; // compensate filter gain loss
        break;
      }
      case 2: // triangle
    ...
    if (l > knee)
      l = knee + (top - knee) * tanhf((l - knee) / (top - knee));
    if (l < -knee)
      l = -knee + (top - knee) * tanhf((l + knee) / (top - knee));
    if (r > knee)
      r = knee + (top - knee) * tanhf((r - knee) / (top - knee));
    if (r < -knee)
      r = -knee + (top - knee) * tanhf((r + knee) / (top - knee));
...
m3ApiRawFunction(host_tone) {
  ...
  if (f0 > 20000.0f)
    f0 = 20000.0f;
  if (f1 > 0.0f && f1 < 1.0f)
    f1 = 0.0f; // degenerate slide target: none
  if (f1 > 20000.0f)
    f1 = 20000.0f;
