...
static const Color DEFAULT_PALETTE[16] = {
    {26, 28, 44, 255},    {93, 39, 93, 255},    {177, 62, 83, 255},
    {239, 125, 87, 255},  {255, 205, 117, 255}, {167, 240, 112, 255},
    {56, 183, 100, 255},  {37, 113, 121, 255},  {41, 54, 111, 255},
    {59, 93, 201, 255},   {65, 166, 246, 255},  {115, 239, 247, 255},
    {244, 244, 244, 255}, {148, 176, 194, 255}, {86, 108, 134, 255},
    {51, 60, 87, 255},
};
...
// 8x8 bitmap font, shared byte-for-byte with the web host
// (cmd/vex-web/assets/vex.js) so text() looks identical in both. Glyphs cover
// ASCII 32..127; FONT8[c - 32] packs one glyph as a 64-bit value where the
// most-significant byte is the top row and the most-significant bit of each
// byte is the left pixel.
...
static void reset_palette(void) {
  for (int i = 0; i < 16; i++)
    g_palette[i] = pack_rgba(DEFAULT_PALETTE[i].r, DEFAULT_PALETTE[i].g,
                             DEFAULT_PALETTE[i].b);
}
...
static void fb_fill(uint32_t c) {
  for (size_t i = 0; i < VEX_W * VEX_H; i++)
    g_fb[i] = c;
}
...
m3ApiRawFunction(host_cls) {
  m3ApiGetArg(int32_t, color) fb_fill(g_palette[(unsigned)(color) & 15]);
  m3ApiSuccess();
}

m3ApiRawFunction(host_pset) {
  m3ApiGetArg(int32_t, x) m3ApiGetArg(int32_t, y) m3ApiGetArg(int32_t, color)
      fb_pset(x, y, g_palette[(unsigned)(color) & 15]);
  m3ApiSuccess();
}

m3ApiRawFunction(host_rect) {
  m3ApiGetArg(int32_t, x) m3ApiGetArg(int32_t, y) m3ApiGetArg(int32_t, w)
      m3ApiGetArg(int32_t, h) m3ApiGetArg(int32_t, color)
...
static void host_bresenham(int32_t x0, int32_t y0, int32_t x1, int32_t y1,
                           uint32_t c) {
...
m3ApiRawFunction(host_line) {
  m3ApiGetArg(int32_t, x0) m3ApiGetArg(int32_t, y0) m3ApiGetArg(int32_t, x1)
      m3ApiGetArg(int32_t, y1)
          m3ApiGetArg(int32_t, color) if (!COORDS_OK(x0, y0) ||
                                          !COORDS_OK(x1, y1)) m3ApiSuccess();
...
m3ApiRawFunction(host_tri) {
  m3ApiGetArg(int32_t, x1) m3ApiGetArg(int32_t, y1) m3ApiGetArg(int32_t, x2)
      m3ApiGetArg(int32_t, y2) m3ApiGetArg(int32_t, x3) m3ApiGetArg(int32_t, y3)
          m3ApiGetArg(int32_t, color) if (!COORDS_OK(x1, y1) ||
                                          !COORDS_OK(x2, y2) ||
                                          !COORDS_OK(x3, y3)) m3ApiSuccess();
  int32_t ymin = y1 < y2 ? (y1 < y3 ? y1 : y3) : (y2 < y3 ? y2 : y3);
  int32_t ymax = y1 > y2 ? (y1 > y3 ? y1 : y3) : (y2 > y3 ? y2 : y3);
  int64_t n64 = (int64_t)ymax - ymin + 1;
  if (n64 <= 0 || n64 > TRI_MAX_ROWS)
    m3ApiSuccess(); // guard hostile spans
  int n = (int)n64;
...
m3ApiRawFunction(host_trib) {
  m3ApiGetArg(int32_t, x1) m3ApiGetArg(int32_t, y1) m3ApiGetArg(int32_t, x2)
      ...
      if (!COORDS_OK(x1, y1) || !COORDS_OK(x2, y2) || !COORDS_OK(x3, y3))
          m3ApiSuccess();
  uint32_t c = g_palette[(unsigned)(color) & 15];
...
m3ApiRawFunction(host_text) {
  m3ApiGetArgMem(const char *, s) m3ApiGetArg(int32_t, x)
      m3ApiGetArg(int32_t, y) m3ApiGetArg(int32_t, color) m3ApiCheckMem(s, 1);
  char buf[128];
...
m3ApiRawFunction(host_btnp) {
  m3ApiReturnType(int32_t) m3ApiGetArg(int32_t, button)
...
m3ApiRawFunction(host_mbtn) {
  m3ApiReturnType(int32_t) m3ApiGetArg(int32_t, button)
      ...
      int held =
          g_window_open && button >= 0 && button < 3 ? IsMouseButtonDown(button)
                                                     : 0;
  m3ApiReturn(held);
}
...
m3ApiRawFunction(host_pal) {
  m3ApiGetArg(int32_t, index) m3ApiGetArg(int32_t, rgb)
...
static void voice_apply(ToneVoice *v, const ToneTrigger *t) {
  ...
  const int frames[4] = {t->frames[0], t->frames[1], t->frames[2],
                         t->frames[3]};
  const double ends[4] = {t->peak, t->sus, t->sus, 0.0};
  long total = 0;
  for (int i = 0; i < 4; i++) {
    v->seg_len[i] = frames[i] > 0 ? (long)((double)frames[i] * spf + 0.5) : 0;
    v->seg_end[i] = ends[i];
    total += v->seg_len[i];
  }
  if (total <= 0) {
  ...
  for (unsigned int pos = 0; pos < frames; pos++) {
    ...
    for (int ch = 0; ch < VEX_TONE_CHANNELS; ch++) {
      ...
      case 1: { // noise: full-period 15-bit LFSR, clock clamped
        ...
        if (nclk > TONE_NOISE_CLK_MAX)
          nclk = TONE_NOISE_CLK_MAX;
        v->nph += nclk / rate;
        while (v->nph >= 1.0) {
          v->nph -= 1.0;
      ...
      default: { // pulse — polyBLEP bandlimited, DC blocked
        ...
        if (t2 >= 1.0)
          t2 -= 1.0;
        raw -= poly_blep(t2, dt);
        // one-pole warmth; polyBLEP already tames alias
        v->lp += 0.5 * (raw - v->lp);
        double y = v->lp;
...
m3ApiRawFunction(host_tone) {
  m3ApiGetArg(int32_t, freq) m3ApiGetArg(int32_t, duration)
      m3ApiGetArg(int32_t, volume)
          m3ApiGetArg(int32_t, flags) if (!g_audio_ready) m3ApiSuccess();
  ensure_stream();

  static const double duty_table[4] = {0.5, 0.25, 0.125, 0.75};
  static const double pan_l[3] = {0.70710678, 1.0, 0.0};
  static const double pan_r[3] = {0.70710678, 0.0, 1.0};
  ...
  if ((flags >> 8) & 1) { // note mode: MIDI note number
    f0 = 440.0 * pow(2.0, ((double)freq - 69.0) / 12.0);
  } else {
    f0 = (double)(freq & 0xFFFF);
    f1 = (double)((freq >> 16) & 0xFFFF);
    if (f0 < 1.0)
      f0 = 1.0;
  }
  if (f0 > 20000.0)
    f0 = 20000.0;
  if (f1 > 0.0 && f1 < 1.0)
    f1 = 0.0; // degenerate slide target: none
  if (f1 > 20000.0)
    f1 = 20000.0;
...
static bool load_cart(IM3Environment env, const char *path, Cart *out) {
  ...
  if (!wasm) {
    fprintf(stderr, "vex: cannot read %s\n", path);
    return false;
  }

  IM3Runtime rt = m3_NewRuntime(env, 64 * 1024 /* stack */, NULL);
  if (!rt) {
    fprintf(stderr, "vex: cannot create runtime\n");
    free(wasm);
    return false;
  }
  IM3Module mod;
...
int main(int argc, char **argv) {
  ...
  bool trace =
      false;        // -t/--trace: with -n, print a line per framebuffer change
  ...
  if (frames >= 0) {
    ...
    if (dump_path) {
      ...
      if (!f) {
        fprintf(stderr, "vex: cannot write %s\n", dump_path);
        return 1;
      }
  ...
  if (screen.id == 0)
    die(cart.rt, "cannot create framebuffer texture", NULL);
  ...
  while (!WindowShouldClose()) {
    ...
    int mon = GetCurrentMonitor();
    bool surface_mismatch =
        IsWindowFullscreen() && (GetMonitorWidth(mon) != GetRenderWidth() ||
    ...
    if (surface_mismatch) {
      ...
      rlLoadIdentity();
      DrawTexturePro(screen, src, (Rectangle){0, 0, (float)VEX_W, (float)VEX_H},
